...
class_name PlaceGivenCardDownEvent
...
func execute(card_ref: Card) -> bool:
	...
	if in_hand:
		...
		card.temporary = true
		RiftGrid.Instance.place_card(selected_card_pos, card)
		# This just need be some function that can summon other cards
		# Currently Affected: Chair Spell / Chair Token, Fake Chest, 
	else:
		# In Rift
